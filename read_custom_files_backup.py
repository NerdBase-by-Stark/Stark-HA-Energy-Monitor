...
def generate_file_tree(directory):
    ...
    for root, dirs, files in os.walk(directory):
        # Skip .git directory
        dirs[:] = [d for d in dirs if d != '.git']  # This line excludes .git from the search
            ...
            for root, dirs, files in os.walk(directory):
                for file in files:
                    if any(file.endswith(ext) for ext in FILE_EXTENSIONS):
                        ...
                        try:
                            # Extract the relative file path (for clarity when files are in subfolders)
                            relative_path = os.path.relpath(filepath, directory)
                            with open(filepath, 'r', encoding='utf-8') as f:
                                # Write the file name with the relative path if it's in a subfolder
